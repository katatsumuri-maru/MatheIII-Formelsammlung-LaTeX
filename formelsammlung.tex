...
$
\int \cos(x) dx = \sin(x) + C \mid \int \sin(x) dx = -\cos(x) + C
$
...
$
\int \frac{1}{\sqrt{1-x^2}} = \arcsin(x) \mid \int \frac{-1}{\sqrt{1-x^2}} = \arccos(x)
$
...
$
\int f'(x)\cdot g(x) \mathrm{d}x = \left[f(x)\cdot g(x)\right]_{a}^{b} - \int f(x)\cdot g'(x)\,\mathrm{d}x\\
$
Wahl von f: ln(x)$->\arcsin(x)$ etc. $->x^3$ etc. $->\sin(x)$ etc $->e^x$
